...
class Classe_Correcao:
    ...
    def importar_teste(self,tabela_teste):
        ...
        planilha_teste = pd.read_csv(tabela_teste, skiprows=[0,1], sep=" ")
        
        global email,nome_crianca, data, nome_responsavel
        
        dados_planilha = pd.read_csv(tabela_teste, nrows=1, sep=" ") # Recupera o endereço de e-mail
        
        email = dados_planilha.iloc[0,0] # Recupera o endereço de e-mail
        nome_crianca = dados_planilha.iloc[0,1]
        data = dados_planilha.iloc[0,2]
        nome_responsavel = dados_planilha.iloc[0,3]
        
        #planilha_teste = planilha_teste.drop(planilha_teste.index[[planilha_teste.shape[0]-1]]) # Apaga a linha com o endereço de e-mail
    ...
    def predicao(self):
        ...
        for i in planilha_teste['Foto'].unique(): 
            ...
            for j in range(len(tabela_selecao)):
                ...
                Foto = int(tabela_selecao.iloc[j]['Foto']) # Número da imagem
        ...
        ##########################################################################
                ...
                if (ponto_x>larg_imagem) or (ponto_y>alt_imagem) or (ponto_x<0) or (ponto_y<0):
                    predicao = ""
                else:
        ...
        ###############################################################################
                ...
                linha = "Ponto = " + str(contador) +" | x = " + str(X) + " | y = " +  str(y) + " | Predição = " + str(predicao)
                resultado_predicao = resultado_predicao+linha+"\n"
                
                contador=contador+1
                
            linha = "Nome do Classificador: " + classif_ranking[Foto-1][2]
            resultado_predicao = resultado_predicao+linha+"\n"
    ...
    def envio_email(self,assunto,mensagem,arquivo):
        ...
        s = smtplib.SMTP('smtp.***.edu.br', 587)
          
        # start TLS for security
        s.starttls()
          
        # Authentication
        s.login(fromaddr, base64.b64decode("**********").decode("utf-8"))
    ...
    def relatorio_PDF(self,texto):
        pdf = FPDF()
        pdf.add_page()
        pdf.set_font('Arial', 'B', 10)
        
        pdf.cell(40, 10, 'Nome da Crianca:')
        pdf.ln()
        pdf.cell(40, 10, nome_crianca)
        pdf.ln()
        pdf.cell(40, 10, 'Data da Captação:')
        pdf.ln()
        pdf.cell(40, 10, data)
        pdf.ln()
        pdf.cell(40, 10, 'Nome do Responsável pela Captação:')
        pdf.ln()
        pdf.cell(40, 10, nome_responsavel)
        pdf.ln()
        pdf.ln()
        pdf.ln()
        pdf.cell(40, 10, 'Pontos Captados:')
        pdf.ln()
        
        for linha in texto.splitlines():
        
            pdf.cell(40, 10, linha)
            pdf.ln()

        pdf.output('relatorio.pdf', 'F')
        
        self.envio_email("Teste","Teste","relatorio.pdf")
    ...
    def acao_botao(self,arquivo):
        ...
        self.relatorio_PDF(resultado)
        
        return str(resultado)
    

#ABC = Classe_Correcao()
#ABC.impressao_teste_2()
